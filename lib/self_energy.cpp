...
Self_energy get_self_energy_born_k_T(band b, Energy ene, kpoint kp, chemical_potential mu, Energy epsilon, double coef) { // {{{
    ...
    auto fn = [=](int band_index, chemical_potential e, kpoint k) {
        ...
        weight = std::exp(- weight * sigma_imp*sigma_imp);

        for(int i=0; i<bandsL; i++) {
            se[i][i] = zi*weight*GR[i][i].imag();
            for(int j=i+1; j<bandsL; j++) {
                se[i][j] = zi * weight * GR[i][j].imag();
                se[j][i] = zi * weight * GR[j][i].imag();
            }
        }
    ...
    integrate_band_no_para_T(fn, self_ene, b, ene + mu);
...
Self_energy get_self_energy_born_k_L(band b, Energy ene, int valley, kpoint kp, chemical_potential mu, Energy epsilon, double coef) { // {{{
    ...
    integrate_band_no_para_L(fn, self_ene, b, valley, ene + mu);
...
vectorReal get_lifetime_Gaussian_T(band b, chemical_potential mu, triangles tri, Energy epsilon, double coef) { // {{{ int size = tri.vertexes.size();
    int size = tri.vertexes.size();
...
//                mtx.unlock();
                ...
                Self_energy se = get_self_energy_born_k_T(b, 0e0, k, mu, epsilon, coef);
...
//                double tau = - hbar / (2e0*gamma.imag());
        ...
        threads[i_thread] = std::thread(func, b, i_thread);
    }
...
//                mtx.unlock();
                ...
                Self_energy se = get_self_energy_born_k_L(b, 0e0, valley, k, mu, epsilon, coef);
