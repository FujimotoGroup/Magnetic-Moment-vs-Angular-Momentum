...
dos_dict = {}
conductivity_dict = {}
spin_magnetic_conductivity1_dict = {}
spin_magnetic_conductivity2_dict = {}
spin_magnetic_conductivity_total_dict = {}
spin_angular_conductivity1_dict = {}
spin_angular_conductivity2_dict = {}
spin_angular_conductivity_total_dict = {}
...
# dos T
    ...
    print(readfile)
    dos_dict[cutoff] = pd.read_csv(readfile,header=0).values

# T conductivity
    readfile = data+'mu-dependence/conductivity_eps'+label+'.csv'
    print(readfile)
    conductivity_dict[cutoff] = pd.read_csv(readfile,header=0).values
...
# T spin magnetic conductivity 1
    ...
    spin_magnetic_conductivity1_dict[cutoff] = conductivity1

# T spin magnetic conductivity 2
    readfile = data+'mu-dependence/spin-magnetic-conductivity2_eps'+label+'.csv'
    print(readfile)
    df = pd.read_csv(readfile,header=0)
    conductivity2 = df.values
    spin_magnetic_conductivity2_dict[cutoff] = conductivity2

# T total spin magnetic Hall conductivity
    conductivity_total = conductivity1 + conductivity2
    conductivity_total[:,0] = conductivity1[:,0]
    spin_magnetic_conductivity_total_dict[cutoff] = conductivity_total
...
def dos(data, filename):
    ...
    for cutoff, dos in data.items():
        axes.scatter(dos[:,0], dos[:,1], s=4, label="numeric"+" "+cutoff)
...
axes = axes.flatten()
window = [0, 0]
for conductivity in conductivity_dict.values():
    upper = conductivity[:,1:].max()*1.1e0
    lower = -0.01e0*conductivity[:,1:].max()
    window = [min(window[0], lower), max(window[1], upper)]
for ax in axes:
    ax.yaxis.set_major_formatter(ScalarFormatter(useMathText=True))
    ax.ticklabel_format(style="sci",  axis="y", scilimits=(0,0))
...
axes[1].set_ylim(window)
i = 0
for cutoff, conductivity in conductivity_dict.items():
    axes[0].scatter(conductivity[:,0], conductivity[:,1], s=3, marker=markers[0], edgecolors=colors[3+3*i], facecolor='None', label="xx"+" "+cutoff)
    axes[0].scatter(conductivity[:,0], conductivity[:,5], s=3, marker=markers[1], edgecolors=colors[2+3*i], facecolor='None', label="yy"+" "+cutoff)
    axes[0].scatter(conductivity[:,0], conductivity[:,9], s=3, marker=markers[3], edgecolors=colors[1+3*i], facecolor='None', label="zz"+" "+cutoff)
    axes[0].plot(conductivity[:,0], conductivity[:,1], color=colors[3+3*i])
    axes[0].plot(conductivity[:,0], conductivity[:,5], color=colors[2+3*i])
    axes[0].plot(conductivity[:,0], conductivity[:,9], color=colors[1+3*i])
    axes[0].legend()

    axes[1].scatter(conductivity[:,0], conductivity[:,2], s=3, marker=markers[0], edgecolors=colors[3+6*i], facecolor='None', label="xy"+" "+cutoff)
    axes[1].scatter(conductivity[:,0], conductivity[:,3], s=3, marker=markers[1], edgecolors=colors[2+6*i], facecolor='None', label="xz"+" "+cutoff)
    axes[1].scatter(conductivity[:,0], conductivity[:,4], s=3, marker=markers[3], edgecolors=colors[1+6*i], facecolor='None', label="yx"+" "+cutoff)
    axes[1].scatter(conductivity[:,0], conductivity[:,6], s=3, marker=markers[2], edgecolors=colors[4+6*i], facecolor='None', label="yz"+" "+cutoff)
    axes[1].scatter(conductivity[:,0], conductivity[:,7], s=3, marker=markers[4], edgecolors=colors[5+6*i], facecolor='None', label="zx"+" "+cutoff)
    axes[1].scatter(conductivity[:,0], conductivity[:,8], s=3, marker=markers[5], edgecolors=colors[6+6*i], facecolor='None', label="zy"+" "+cutoff)
    axes[1].plot(conductivity[:,0], conductivity[:,2], color=colors[3+6*i])
    axes[1].plot(conductivity[:,0], conductivity[:,3], color=colors[2+6*i])
    axes[1].plot(conductivity[:,0], conductivity[:,4], color=colors[1+6*i])
    axes[1].plot(conductivity[:,0], conductivity[:,6], color=colors[4+6*i])
    axes[1].plot(conductivity[:,0], conductivity[:,7], color=colors[5+6*i])
    axes[1].plot(conductivity[:,0], conductivity[:,8], color=colors[6+6*i])
    axes[1].legend()
    i += 1
...
maximun = 0
for conductivity in spin_magnetic_conductivity1_dict.values():
    maximun = max(maximun, np.abs(conductivity[:,1:].max()*1.1e0))
for conductivity in spin_magnetic_conductivity2_dict.values():
    maximun = max(maximun, np.abs(conductivity[:,1:].max()*1.1e0))
window = [-maximun, maximun]
# T spin magnetic conductivity 1
# 1,4 layout -  {{{
...
axes[0].set_xlabel("mu [eV]")
axes[0].set_ylim(window)
for cutoff, conductivity1 in spin_magnetic_conductivity1_dict.items():
    for key, j in plot_dict1.items():
        axes[0].scatter(conductivity1[:,0], conductivity1[:,j], s=7, marker=markers[key], edgecolors=colors[key], facecolor='None', label=titles[j]+" "+cutoff)
axes[0].legend()

left, bottom, width, height = [0.2, 0.6, 0.35, 0.35]
ax0 = axes[0].inset_axes([left, bottom, width, height])
ax0.set_xlim([-float(cutoffs[0]), -float(cutoff[0])+0.01])
ax0.yaxis.set_major_formatter(ScalarFormatter(useMathText=True))
ax0.ticklabel_format(style="sci",  axis="y", scilimits=(0,0))
for cutoff, conductivity1 in spin_magnetic_conductivity1_dict.items():
    for key, j in plot_dict1_inset.items():
        ax0.scatter(conductivity1[:,0], conductivity1[:,j], s=7, marker=markers[key], edgecolors=colors[key], facecolor='None', label=titles[j]+" "+cutoff)
axes[0].indicate_inset_zoom(ax0, edgecolor="black")
...
for cutoff, conductivity1 in spin_magnetic_conductivity1_dict.items():
    for key, j in plot_dict2.items():
        axes[1].scatter(conductivity1[:,0], conductivity1[:,j], s=7, marker=markers[key], edgecolors=colors[key], facecolor='None', label=titles[j]+" "+cutoff)
axes[1].legend()

left, bottom, width, height = [0.2, 0.1, 0.3, 0.3]
ax1 = axes[1].inset_axes([left, bottom, width, height])
ax1.set_xlim([-float(cutoffs[0]), -float(cutoff[0])+0.01])
ax1.yaxis.set_major_formatter(ScalarFormatter(useMathText=True))
ax1.ticklabel_format(style="sci",  axis="y", scilimits=(0,0))
for cutoff, conductivity1 in spin_magnetic_conductivity1_dict.items():
    for key, j in plot_dict2_inset.items():
        ax1.scatter(conductivity1[:,0], conductivity1[:,j], s=7, marker=markers[key], edgecolors=colors[key], facecolor='None', label=titles[j]+" "+cutoff)
axes[1].indicate_inset_zoom(ax1, edgecolor="black")
...
for cutoff, conductivity1 in spin_magnetic_conductivity1_dict.items():
    for key, j in plot_dict3.items():
        axes[2].scatter(conductivity1[:,0], conductivity1[:,j], s=7, marker=markers[key], edgecolors=colors[key], facecolor='None', label=titles[j]+" "+cutoff)
axes[2].legend()

left, bottom, width, height = [0.2, 0.1, 0.3, 0.3]
ax2 = axes[2].inset_axes([left, bottom, width, height])
ax2.set_xlim([-float(cutoffs[0]), -float(cutoff[0])+0.01])
ax2.yaxis.set_major_formatter(ScalarFormatter(useMathText=True))
ax2.ticklabel_format(style="sci",  axis="y", scilimits=(0,0))
for cutoff, conductivity1 in spin_magnetic_conductivity1_dict.items():
    for key, j in plot_dict3.items():
        ax2.scatter(conductivity1[:,0], conductivity1[:,j], s=7, marker=markers[key], edgecolors=colors[key], facecolor='None', label=titles[j]+" "+cutoff)
axes[2].indicate_inset_zoom(ax2, edgecolor="black")
...
for cutoff, conductivity1 in spin_magnetic_conductivity1_dict.items():
    for key, j in plot_dict4a.items():
        axes[3].scatter(conductivity1[:,0], conductivity1[:,j], s=7, marker=markers[key], edgecolors=colors[key], facecolor='None', label=titles[j]+" "+cutoff)
    for key, j in plot_dict4b.items():
        axes[3].scatter(conductivity1[:,0], conductivity1[:,j], s=7, marker=markers[key], c=colors[key+len(plot_dict4a)], label=titles[j]+" "+cutoff)
axes[3].legend()

left, bottom, width, height = [0.2, 0.1, 0.3, 0.3]
ax3 = axes[3].inset_axes([left, bottom, width, height])
ax3.set_xlim([-float(cutoffs[0]), -float(cutoff[0])+0.01])
ax3.yaxis.set_major_formatter(ScalarFormatter(useMathText=True))
ax3.ticklabel_format(style="sci",  axis="y", scilimits=(0,0))
for cutoff, conductivity1 in spin_magnetic_conductivity1_dict.items():
    for key, j in plot_dict4a.items():
        ax3.scatter(conductivity1[:,0], conductivity1[:,j], s=7, marker=markers[key], edgecolors=colors[key], facecolor='None', label=titles[j]+" "+cutoff)
    for key, j in plot_dict4b.items():
        axes[3].scatter(conductivity1[:,0], conductivity1[:,j], s=7, marker=markers[key], c=colors[key+len(plot_dict4a)], label=titles[j]+" "+cutoff)
axes[3].indicate_inset_zoom(ax3, edgecolor="black")
...
axes[0].set_xlabel("mu [eV]")
axes[0].set_ylim(window)
for cutoff, conductivity1 in spin_magnetic_conductivity2_dict.items():
    for key, j in plot_dict1.items():
        axes[0].scatter(conductivity1[:,0], conductivity1[:,j], s=7, marker=markers[key], edgecolors=colors[key], facecolor='None', label=titles[j]+" "+cutoff)
axes[0].legend()
...
# 1,4 layout -  {{{
maximun = 0
for conductivity in spin_magnetic_conductivity_total_dict.values():
    maximun = max(maximun, np.abs(conductivity[:,1:].max()*1.1e0))
