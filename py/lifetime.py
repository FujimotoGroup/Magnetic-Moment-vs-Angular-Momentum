...
print("T: max = "+str(np.max(T[:,6]))+", min = "+str(np.min(T[:,6])))
lifetime = ax.scatter(T[:,0], T[:,1], T[:,2], c=T[:,6], cmap=matplotlib.cm.Blues)
fig.colorbar(lifetime)
...
print("L1: max = "+str(np.max(L1[:,6]))+", min = "+str(np.min(L1[:,6])))
lifetime = ax.scatter(L1[:,0], L1[:,1], L1[:,2], c=L1[:,6], cmap=matplotlib.cm.Blues)
cbarL1 = fig.colorbar(lifetime)
...
print("L2: max = "+str(np.max(L2[:,6]))+", min = "+str(np.min(L2[:,6])))
lifetime = ax.scatter(L2[:,0], L2[:,1], L2[:,2], c=L2[:,6], cmap=matplotlib.cm.Blues)
fig.colorbar(lifetime)
...
print("L3: max = "+str(np.max(L3[:,6]))+", min = "+str(np.min(L3[:,6])))
lifetime = ax.scatter(L3[:,0], L3[:,1], L3[:,2], c=L3[:,6], cmap=matplotlib.cm.Blues)
fig.colorbar(lifetime)
...
tau = np.concatenate([T, L1, L2, L3])
lifetime = ax1.scatter(tau[:,0], tau[:,1], tau[:,2], c=tau[:,6])
fig.colorbar(lifetime)
plt.show()
exit()
